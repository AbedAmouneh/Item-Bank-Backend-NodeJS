...
import { AssignUsersSchema } from '../models';
...
export async function assignUsers(
  ...
  try {
    ...
    const body = AssignUsersSchema.parse(request.body);
    await service.assignUsers(id, body, request.user.id, request.user.tenant_id);
    ...
    logger.error({ error }, 'POST /assignments/:id/users failed');
