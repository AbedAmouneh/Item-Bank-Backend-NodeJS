// controllers/assessmentsController/handlers/patch_assessment.ts
import { FastifyReply } from 'fastify';
import { ZodError } from 'zod';
import { AuthenticatedRequest } from '../../../platform/http/middlewares/auth';
import { createChildLogger } from '../../../utils/logger';
import { UpdateAssessmentSchema } from '../models';
...
export async function updateAssessment(
  ...
  try {
    ...
    const assessment = await service.updateAssessment(id, body, request.user.tenant_id);
    ...
    logger.error({ error }, 'PATCH /assessments/:id failed');
